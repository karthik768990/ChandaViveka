export function matchChandas(detectedPattern, chandasList) {
  let bestMatch = { name: "Unknown / Mixed", confidence: 0, matchedPattern: null };

  for (const ch of chandasList) {
    const expected = ch.pattern[0];
    const perPada = ch.syllables_per_pada;
    const padaPatterns = detectedPattern.match(new RegExp(`.{1,${perPada}}`, "g")) || [];
    ...
    if (overallScore > bestMatch.confidence) {
      bestMatch = { name: ch.name, confidence: overallScore, matchedPattern: expected };
    }
  }

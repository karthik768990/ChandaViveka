...
for (const chandas of dbChandas) {
  ...
  // Generate a repeated form to compare long pādas
  const repeated = base.repeat(Math.ceil(lgPatterns.join('').length / base.length));

  // Compute similarity ratio
  const inputCombined = lgPatterns.join('');
  const distance = levenshtein(inputCombined, repeated.slice(0, inputCombined.length));
  const similarity = 1 - distance / inputCombined.length;

  if (similarity >= 0.7) { // 70% or higher considered a match
    return {
      identifiedChandas: name,
      explanation: `The input pattern (${inputCombined.length} syllables) closely resembles ${name} (${(similarity*100).toFixed(1)}% match) → canonical pattern ${base}.`,
    };
  }
}

...
// ===================================================================
// 🌸 Sanskrit Prosody (Chandas) Analyzer — Final Robust Version
// ===================================================================
...
// ===================================================================
// Pattern Matcher — with fuzzy similarity
// ===================================================================
...
const findMatchInDb = (lgPatterns, dbChandas) => {
  ...
  // ---------------------------------------------------------------------
  const padaLength = Math.round(
    lgPatterns.reduce((a, p) => a + p.length, 0) / lgPatterns.length
  );

  // ---------------------------------------------------------------------
  // 🔹 2️⃣ Try direct or syllable-filtered fuzzy match
  // ---------------------------------------------------------------------
  for (const chandas of dbChandas) {
    ...
    const canonicalLength = base.length;

    // Skip if syllable length too different
    if (Math.abs(canonicalLength - padaLength) > 3) continue;

    const repeated = base.repeat(Math.ceil(combined.length / base.length));
    const inputCombined = combined;
    const distance = levenshtein(
      inputCombined,
      repeated.slice(0, inputCombined.length)
    );
    const similarity = 1 - distance / inputCombined.length;

    if (similarity >= 0.75) {
      return {
        identifiedChandas: name,
        explanation: `Matched ${name} (≈${(
          similarity * 100
        ).toFixed(1)}% match) — syllable count per pāda ≈ ${padaLength}, canonical ${canonicalLength}.`,
      };
    }
  }

  // ---------------------------------------------------------------------
  // 🔹 3️⃣ Check for special case: Anuṣṭubh (8-syllable pādas)
  // ---------------------------------------------------------------------
  if (combined.length % 8 === 0) {
    ...
    if (isAnushtubh) {
      return {
        ...
        explanation:
          "Matches Anuṣṭubh pattern (8-syllable pādas with 5th Laghu, 6th Guru).",
      };
    }
  }

  // ---------------------------------------------------------------------
  // 🔹 4️⃣ Fuzzy fallback — check near matches with higher tolerance
  // ---------------------------------------------------------------------
  const closeMatches = [];
  for (const chandas of dbChandas) {
    const { name, pattern } = chandas;
    ...
    const repeated = pattern.repeat(Math.ceil(combined.length / pattern.length));
    const distance = levenshtein(
      combined,
      repeated.slice(0, combined.length)
    );
  ...
  if (closeMatches.length > 0) {
    return {
      ...
      explanation:
        "Pattern closely resembles one or more known Chandas (minor variations possible).",
    };
  }

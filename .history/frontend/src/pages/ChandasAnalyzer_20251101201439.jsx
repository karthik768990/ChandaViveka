...
const findMatchInDb = (lgPattern, dbChandas) => {
  ...
  for (const chandas of dbChandas) {
    // --- THIS IS THE FIX ---
    // We only destructure 'name' and 'pattern'
    const { name, pattern } = chandas;
    
    // Skip if the DB entry has no pattern
    if (!pattern) continue;
    ...
    if (lgPattern.length > 0 && lgPattern.length % pattern.length === 0) {
      ...
      if (pattern.repeat(repeats) === lgPattern) {
        return { 
          ...
          // --- THIS IS THE FIX ---
          // We generate our own explanation string
  ...
  if (lgPattern.length > 0 && lgPattern.length % 8 === 0) {
    ...
    for (const pada of padas) {
      // 5th must be Laghu, 6th must be Guru
    ...
    if (isAnushtubh) {
      return {
        ...
        // --- THIS IS THE FIX ---
        // We use our hardcoded, reliable explanation for Anuṣṭubh
